<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
    ...
    <div class="flex justify-between items-center mb-4">
        <div>
            ...
            <p class="text-gray-600">Total: {{ $vistausuarios->total() }} usuarios</p>
        </div>
        <div class="flex items-center space-x-4">
            <p class="text-gray-600">Filtrar: </p>
            <div class="flex items-center space-x-2">
                <button wire:click="filtrarTipo('todos')"
                    class="px-3 py-1 rounded-l-full text-white hover:bg-black {{ $filterTipo === null ? 'bg-emerald-600' : 'bg-blue-500' }}">
                    Todos
                </button>
                <button wire:click="filtrarTipo('cliente')"
                    class="px-3 py-1 text-white hover:bg-black {{ $filterTipo === 'cliente' ? 'bg-emerald-600' : 'bg-blue-500' }}">
                    Cliente
                </button>
                <button wire:click="filtrarTipo('empresa')"
                    class="px-3 py-1 rounded-r-full text-white hover:bg-black {{ $filterTipo === 'empresa' ? 'bg-emerald-600' : 'bg-blue-500' }}">
                    Empresa
                </button>
            </div>
            <!-- Buscador -->
            ...
            <button wire:click="abrirModalCrear"
                class="bg-yellow-300 hover:bg-black text-white font-semibold px-4 py-2 rounded-lg shadow flex items-center">
            ...
            <tbody class="text-gray-700">
                @forelse ($vistausuarios as $user)
                    <tr class="border-b hover:bg-gray-50">
                        ...
                        <td class="p-4">
                            {{ $user->personas?->dni ?? 'Sin DNI' }}
                        </td>
                        <td class="p-4">{{ $user->email }}</td>
                        <td class="p-4">
                            <span class="px-2 py-1 bg-blue-100 text-blue-800 rounded-full text-xs">
                                {{ $user->tipousuarios?->tipousu ?? 'Sin tipo' }}
                            </span>
                        </td>
                        <td class="p-4">
                        ...
                        <td class="p-4 flex items-center">
                            <button
                                class="px-3 py-1 rounded-l-full text-white {{ $user->estado_usu == 1 ? 'bg-emerald-500' : 'bg-gray-300' }}"
                    ...
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <select wire:model="form.fk_idtipousuarios"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                                    <option value="">Seleccione...</option>
                                    @foreach ($tiposUsuario as $tipo)
                                        <option value="{{ $tipo->id }}">{{ $tipo->tipousu }}</option>
                                    @endforeach
                                </select>
                                @error('form.fk_idtipousuarios')
                                    <span class="text-red-500 text-xs">{{ $message }}</span>
                                @enderror
                            </div>
